...
function validatePath(target: string): string {
  ...
  if (!allowed) {
    throw new McpError(
      ...
      `Access denied: "${resolved}" is not within an allowed directory (${allowedDirs.join(", ")})`,
    );
  }
  return resolved;
}
  ...
  tools: [
    {
      name: "read_file",
      description:
        "Read the complete contents of a file. Returns the file content as a string. Use this to inspect source code, configuration files, logs, etc.",
      inputSchema: {
        type: "object",
        properties: {
          path: {
          ...
          encoding: {
            ...
            enum: ["utf8", "base64"],
            description:
              'Text encoding to use. Defaults to "utf8". Use "base64" for binary files.',
            default: "utf8",
          },
        },
        required: ["path"],
    ...
    },
    {
      name: "write_file",
      description:
        "Create or overwrite a file with the supplied content. Parent directories are created automatically if they do not exist.",
      inputSchema: {
        type: "object",
        properties: {
          path: {
          ...
          encoding: {
            ...
            enum: ["utf8", "base64"],
            description:
              'Encoding of the supplied content. Defaults to "utf8".',
            default: "utf8",
          },
        },
    ...
    },
    {
      name: "list_directory",
      description:
        "List the contents of a directory. Returns an array of entries with name, type (file/directory), size, and last-modified date.",
      inputSchema: {
        type: "object",
        properties: {
          path: {
          ...
          recursive: {
            type: "boolean",
            description:
              "If true, list all descendants recursively. Defaults to false.",
            default: false,
          },
        },
        required: ["path"],
      ...
      inputSchema: {
        ...
        properties: {
          ...
          recursive: {
            type: "boolean",
            description:
              "If true, delete directory and all its contents recursively. Defaults to false.",
            default: false,
          },
        },
        required: ["path"],
    ...
    },
    {
      name: "move_file",
      description:
        "Move or rename a file or directory. Overwrites the destination if it already exists and is a file.",
      inputSchema: {
        type: "object",
        properties: {
          source: {
    ...
    },
    {
      name: "get_file_info",
      description:
        "Return metadata about a file or directory: size, creation time, modification time, type, and permissions.",
      inputSchema: {
        type: "object",
        properties: {
          path: {
    ...
    },
    {
      name: "search_files",
      description:
        "Recursively search for files whose names match a glob-style pattern within a directory.",
      inputSchema: {
        type: "object",
        properties: {
          directory: {
          ...
          pattern: {
            type: "string",
            description:
              "Case-insensitive substring or glob-style pattern to match against file names. e.g. \"*.ts\", \"config\".",
          },
          maxResults: {
            type: "number",
    ...
    },
    {
      ...
      description:
        "Search for files whose contents contain the given text (case-insensitive). Returns file paths and matching lines.",
      inputSchema: {
        type: "object",
        properties: {
          directory: {
          ...
          filePattern: {
            type: "string",
            description:
              "Optional pattern to restrict which file names are searched. e.g. \"*.ts\".",
          },
          maxResults: {
            type: "number",
...
server.setRequestHandler(CallToolRequestSchema, async req => {
  ...
  try {
    switch (name) {
      ...
      // -----------------------------------------------------------------------
      case "delete_file": {
        const target = validatePath(args!.path as string);
        const recursive = (args!.recursive as boolean) ?? false;
      ...
      // -----------------------------------------------------------------------
      case "move_file": {
        const src = validatePath(args!.source as string);
        const dest = validatePath(args!.destination as string);
      ...
      // -----------------------------------------------------------------------
      case "search_files": {
        const dirPath = validatePath(args!.directory as string);
  ...
  async function walk(current: string): Promise<void> {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        if (filePattern && !matchesPattern(item.name, filePattern)) continue;
        try {
          const text = await fs.readFile(fullPath, "utf8");
          const lines = text.split("\n");
          const matching: { lineNumber: number; text: string }[] = [];
...
async function main() {
  ...
  process.stderr.write(
    `[mcp-filesystem-server] started. Allowed directories: ${allowedDirs.join(", ")}\n`,
  );
}

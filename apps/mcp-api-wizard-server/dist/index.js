...
function validateSpec(spec) {
    ...
    if (!spec.info?.title)
        problems.push("spec.info.title is required");
    if (!spec.info?.version)
        problems.push("spec.info.version is required");
        ...
        for (const [method, operation] of Object.entries(pathItem)) {
            ...
            for (const pp of pathParams) {
                if (!declaredParams.includes(pp)) {
                    problems.push(`${method.toUpperCase()} ${pathKey}: path parameter "{${pp}}" is not declared in parameters`);
                }
            }
        }
    }
        ...
        const req = lib.request({
            ...
            res.on("end", () => {
                resolve({
                    ...
                    headers: Object.fromEntries(Object.entries(res.headers).map(([k, v]) => [
                        k,
...
function generateMockResponse(operation, preferredStatus) {
    const statuses = Object.keys(operation.responses ?? {}).map(Number).filter(Boolean);
    const status = statuses.includes(preferredStatus)
        ? preferredStatus
...
function generateMockValue(schema, depth = 0) {
    if (depth > 5)
        return null;
    ...
    switch (type) {
        case "object": {
            ...
            return Object.fromEntries(Object.entries(props).map(([k, v]) => [k, generateMockValue(v, depth + 1)]));
        }
        case "array": {
...
// ---------------------------------------------------------------------------
function generateTypeScriptClient(spec, pathKey, method) {
    const op = spec.paths[pathKey]?.[method];
    if (!op)
        return `// endpoint ${method.toUpperCase()} ${pathKey} not found in spec`;
...
function generatePythonClient(spec, pathKey, method) {
    const op = spec.paths[pathKey]?.[method];
    if (!op)
        return `# endpoint ${method.toUpperCase()} ${pathKey} not found in spec`;
    const baseUrl = spec.servers?.[0]?.url ?? "https://api.example.com";
...
function generateCurlExample(spec, pathKey, method) {
    const op = spec.paths[pathKey]?.[method];
    if (!op)
        return `# endpoint ${method.toUpperCase()} ${pathKey} not found in spec`;
    const baseUrl = spec.servers?.[0]?.url ?? "https://api.example.com";
...
// Mock server handler
// ---------------------------------------------------------------------------
function createMockHandler(spec) {
    return (req, res) => {
...
// Server setup
// ---------------------------------------------------------------------------
const server = new Server({ name: "api-wizard", version: "0.1.0" }, { capabilities: { tools: {} } });
// ---------------------------------------------------------------------------
// Tool definitions
// ---------------------------------------------------------------------------
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "create_spec",
            description: "Create a new in-memory OpenAPI 3.1 specification. Returns a specId that must be passed to subsequent tools.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "add_endpoint",
            description: "Add or replace an endpoint (path + HTTP method) in an in-memory spec.",
            inputSchema: {
                type: "object",
                properties: {
                    specId: { type: "string" },
        ...
        {
            name: "validate_spec",
            description: "Validate an in-memory spec for structural and semantic errors. Returns a list of problems or confirms the spec is valid.",
            inputSchema: {
                type: "object",
                properties: {
                    specId: { type: "string" },
        ...
        {
            name: "diff_specs",
            description: "Compare two in-memory specs and report added, removed, and changed endpoints.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "make_request",
            description: "Execute an HTTP request and return the status, headers, and body. Useful for testing live APIs.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "inspect_api",
            description: "Send a request to a live endpoint and automatically infer an OpenAPI schema from the response. Optionally adds the inferred endpoint to an existing spec.",
            inputSchema: {
                type: "object",
                properties: {
                    method: { type: "string" },
        ...
        },
        {
            ...
            description: "Parse a curl command string into a structured request definition. Optionally executes the request.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Generate client code for one or all endpoints in a spec. Supports TypeScript, Python, and curl.",
            inputSchema: {
                type: "object",
                properties: {
                    specId: { type: "string" },
        ...
        },
        {
            ...
            description: "Start an in-process HTTP mock server for a spec on a local port. The mock server returns auto-generated responses based on the spec schemas.",
            inputSchema: {
                type: "object",
                properties: {
                    specId: { type: "string" },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    function requireSpec(specId) {
        const spec = specRegistry.get(specId);
        if (!spec) {
            const available = [...specRegistry.keys()].join(", ") || "(none)";
            throw new McpError(ErrorCode.InvalidParams, `Spec "${specId}" not found. Available specs: ${available}`);
        }
        return spec;
    }
    try {
        switch (name) {
            ...
            case "create_spec": {
                ...
                const serverUrl = args.serverUrl ?? "https://api.example.com";
            ...
            // -----------------------------------------------------------------------
            case "load_spec": {
                const filePath = resolveWorkspacePath(args.filePath);
                ...
                const specId = args.specId ?? generateSpecId(spec.info?.title ?? "api");
                specRegistry.set(specId, spec);
                return {
                    content: [
                        {
            ...
            case "list_specs": {
                const list = [...specRegistry.entries()].map(([id, s]) => ({
                    ...
                    endpointCount: Object.values(s.paths).reduce((acc, p) => acc + Object.keys(p).length, 0),
                }));
                return {
                    content: [
            ...
            case "add_endpoint": {
                ...
                // Build operation
                const operation = {
                    responses: Object.fromEntries(Object.entries(responses).map(([status, resp]) => [
                        status,
                        {
            ...
            case "list_endpoints": {
                ...
                for (const [pathKey, pathItem] of Object.entries(spec.paths)) {
                    for (const [method, op] of Object.entries(pathItem)) {
                        const operation = op;
                        if (tagFilter && !operation.tags?.includes(tagFilter))
                            continue;
                        endpoints.push({
                            method: method.toUpperCase(),
                            path: pathKey,
            ...
            case "get_endpoint": {
                ...
                if (!op) {
                    throw new McpError(ErrorCode.InvalidParams, `Endpoint ${method.toUpperCase()} ${endpointPath} not found in spec "${specId}".`);
                }
                return {
            ...
            // -----------------------------------------------------------------------
            case "diff_specs": {
                const specIdA = args.specIdA;
                const specIdB = args.specIdB;
                const specA = requireSpec(specIdA);
                const specB = requireSpec(specIdB);
                const endpointsA = new Set();
                const endpointsB = new Set();
                for (const [p, pi] of Object.entries(specA.paths)) {
                    for (const m of Object.keys(pi))
                        endpointsA.add(`${m.toUpperCase()} ${p}`);
                }
                for (const [p, pi] of Object.entries(specB.paths)) {
                    for (const m of Object.keys(pi))
                        endpointsB.add(`${m.toUpperCase()} ${p}`);
                }
                ...
                for (const endpoint of common) {
                    ...
                    const opA = specA.paths[pathKey]?.[method.toLowerCase()];
                    const opB = specB.paths[pathKey]?.[method.toLowerCase()];
            ...
            case "inspect_api": {
                ...
                const urlParsed = new URL(args.url);
                const endpointPath = args.endpointPath ?? urlParsed.pathname;
                const method = args.method.toLowerCase();
                const inferred = {
                    summary: `${args.method.toUpperCase()} ${endpointPath}`,
                    responses: {
                ...
                if (args.specId) {
                    const spec = requireSpec(args.specId);
                    if (!spec.paths[endpointPath])
                        spec.paths[endpointPath] = {};
            ...
            case "import_from_curl": {
                const parsed = parseCurlCommand(args.curl);
            ...
            case "add_security_scheme": {
                const specId = args.specId;
                const spec = requireSpec(specId);
                const schemeName = args.name;
                const type = args.type;
                const scheme = { type };
                if (args.scheme)
                    scheme.scheme = args.scheme;
                if (args.bearerFormat)
                    scheme.bearerFormat = args.bearerFormat;
                if (args.in)
                    scheme.in = args.in;
                if (args.keyName)
                    scheme.name = args.keyName;
                if (!spec.components)
                    spec.components = {};
...
async function main() {
    ...
    process.stderr.write(`[mcp-api-wizard-server] started. Workspace: ${workspaceRoot}\n`);
}
main().catch(err => {
